...
public class Voronoi {
    ...
    private void qsort(Site[] sites) {
        ...
        Collections.sort(listSites, new Comparator<Site>() {
            ...
            public final int compare(Site p1, Site p2) {
                Vector2D s1 = p1.coord, s2 = p2.coord;
                if (s1.y < s2.y) {
                    return (-1);
                }
                if (s1.y > s2.y) {
    ...
    private void sortNode(double xValues[], double yValues[], int numPoints) {
        ...
        for (i = 0; i < nsites; i++) {
            sites[i] = new Site();
            sites[i].coord = new Vector2D(xValues[i], yValues[i]);
            sites[i].sitenbr = i;
    ...
    private Vector2D PQ_min() {
        Vector2D answer = new Vector2D(0, 0);
    ...
    private Halfedge ELleftbnd(Vector2D p) {
        int i, bucket;
        Halfedge he;
    ...
    private boolean right_of(Halfedge el, Vector2D p) {
        Edge e;
        Site topsite;
